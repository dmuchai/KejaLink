...
const getListings = async (filters?: SearchFilters & { agentId?: string }) => {
  // Start building the query with eager loading of related data
  let query = supabase
    .from('property_listings')
    .select(`
      *,
      agent:users!agent_id(id, full_name, email, phone_number, is_verified_agent, profile_picture_url),
      images:property_images(id, url, display_order, ai_scan)
    `)
...
const getListingById = async (id: string) => {
  // Fetch single listing with eager loading of agent and images
